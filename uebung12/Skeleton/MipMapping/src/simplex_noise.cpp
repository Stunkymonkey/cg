...
///	This method is faster than using (int32_t)std::floor(fp). I measured it to be approximately twice as fast:
...
/// This inline function costs around 1ns, and is called N+1 times for a noise of N dimension. Using a real hash
///		function would be better to improve the "repeatability of 256" of the above permutation table, but fast integer
///		hash functions uses more time and have bad random properties.
...
/// @note These generate gradients of more than unit length. To make a close match with the value range of classic
///		Perlin noise, the final noise values need to be rescaled to fit nicely within [-1,1]. (The simplex noise
///		functions as such also have different scaling.) Note also that these noise functions are the most practical and
///		useful signed version of Perlin noise.
/// @param hash Hash value.
/// @param x Distance to the corner.
...
/// @param hash Hash value.
/// @param x x-coord of the distance to the corner.
/// @param y y-coord of the distance to the corner.
/// @return Gradient value.
...
/// @param x x-coord of the place to evaluate.
/// @param y y-coord of the place to evaluate.
/// @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
...
/// @param frequency Frequency of the first octave of noise. Further octaves have a higher frequency.
/// @param lacunarity Factor multiplied on the frequency with each further octave. (Try 2.0f)
/// @param persistence Factor multiplied on the noise value (amplitude) with each further octave. (Try 0.5f)
/// @param octaves Number of overlaid octaves of noise.
/// @param x Place to evaluate.
/// @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
/// @pre octaves must be positive (zero isn't allowed, too).
